...
func main() {
	...
	e.POST("/", httpGetStoreByGeo)

	go e.Logger.Fatal(e.Start("0.0.0.0:" + port))
}

func httpGetStoreByGeo(c echo.Context) error {
	req := c.Request()
	decoder := json.NewDecoder(req.Body)

	var geoModel model.Geo

	err := decoder.Decode(&geoModel)
	manageErr(err)
	return c.JSON(http.StatusOK, getStoresByGeo(geoModel.Latitude, geoModel.Longitude, geoModel.RangeMeter))
}
